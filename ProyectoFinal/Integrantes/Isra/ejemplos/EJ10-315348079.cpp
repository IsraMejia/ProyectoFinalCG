...
// Variables para controlar el estado de las teclas de movimiento de la nave
...
//NEW// Keyframes -- Variables que se tienen que controlar por el tecaldo para mover la nave
float posXavion = 2.0, posYavion = 2.0, posZavion = 0;
float	movAvion_x = 0.0f, movAvion_y = 0.0f;
float giroAvion = 0;

#define MAX_FRAMES 100
int i_max_steps = 180; //Interpolaciones entre cada cuadro (aumentado para animación más lenta)
int i_curr_steps = 6;
...
int FrameIndex = 7;			// Inicializado en 7 porque hay 7 keyframes predefinidos (0-6)
bool play = false;
int playIndex = 0;
...
void saveFrame(void) //tecla L
{
	...
	KeyFrame[FrameIndex].giroAvion = giroAvion;  // Corregido: faltaba asignar el valor
...
int main()
{
	...
	//Luz ambiental blanca al 70% de intensidad
	...
	printf("CONTROL DE NAVE/AVION:\n");
	printf("  - Flecha ARRIBA: Mover nave hacia arriba (Y+)\n");
	printf("  - Flecha ABAJO: Mover nave hacia abajo (Y-)\n");
	printf("  - Flecha DERECHA: Mover nave hacia la derecha (X+)\n");
	printf("  - Flecha IZQUIERDA: Mover nave hacia la izquierda (X-)\n");
	printf("  - Tecla N: Girar nave en sentido antihorario\n");
	printf("  - Tecla M: Girar nave en sentido horario\n\n");
	printf("ANIMACION:\n");
	printf("  1. Presionar barra espaciadora para reproducir animacion\n");
	printf("  2. Presionar 0 para volver a habilitar reproduccion de la animacion\n");
	printf("  3. Presiona L para guardar frame\n");
	printf("  4. Presiona P para habilitar guardar nuevo frame\n");
	printf("  5. Presiona 1 para mover en X\n");
	printf("  6. Presiona 2 para habilitar mover en X\n");
...
void inputKeyframes(bool* keys)
{
	// Control de la nave/avion con teclas de flecha
	// Flecha ARRIBA: mueve en Y positivo
	if (keys[GLFW_KEY_UP])
	{
		movAvion_y += 0.1f;
		teclaArribaPresionada = true;
	}
	else if (teclaArribaPresionada)
	{
		printf("Nave Y: %f\n", movAvion_y);
		teclaArribaPresionada = false;
	}
	
	// Flecha ABAJO: mueve en Y negativo
	...
	// Flecha DERECHA: mueve en X positivo
	...
	// Flecha IZQUIERDA: mueve en X negativo
	...
	// Control de giro del avion
	// Tecla N: gira en sentido antihorario
	if (keys[GLFW_KEY_N])
	{
		giroAvion += 2.0f;
		if (giroAvion >= 360.0f)
	...
	else if (teclaNPresionada)
	{
		printf("Giro Avion: %f grados\n", giroAvion);
		teclaNPresionada = false;
	}
	
	// Tecla M: gira en sentido horario
	if (keys[GLFW_KEY_M])
	{
		giroAvion -= 2.0f;
		if (giroAvion <= -360.0f)
